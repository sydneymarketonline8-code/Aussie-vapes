...
begin
  ...
  if v_rating < 1 or v_rating > 5 then raise exception 'rating must be between 1 and 5'; end if;
  if coalesce(trim(payload->>'reviewer_name'), '') = '' then raise exception 'reviewer_name is required'; end if;
  if coalesce(trim(payload->>'body'), '') = '' then raise exception 'body is required'; end if;
