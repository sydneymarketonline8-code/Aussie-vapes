...
export function buildProductFaqs(product: Product, categoryName?: string): ProductFaqItem[] {
  ...
  faqs.push({
    ...
    answer:
      `Yes — the ${product.name} is ${ratingDesc} by VapeHub Vapes Australia customers, averaging ${product.rating.toFixed(
        1
      )}/5 across ${product.reviewCount.toLocaleString()} reviews. ${product.shortDescription} It's stocked authentic at VapeHub Vapes Australia and dispatched same-day from our Sydney warehouse.`,
  })
  ...
  if (isPack && packSize) {
    faqs.push({
      ...
      answer:
        `The ${product.name} includes ${packSize}× ${product.brand} devices in one bundle, dispatched together from the VapeHub Vapes Australia Sydney warehouse. Pack pricing saves you ${
  ...
  faqs.push({
    ...
    answer:
      `Yes — the ${product.name} sold by VapeHub Vapes Australia is TGA-compliant 20mg/mL (2%) salt nicotine and operates within Australia's prescription model for nicotine vaping products. You confirm prescription status at checkout.`,
  })
