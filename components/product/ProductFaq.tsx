...
export function buildProductFaqs(product: Product, categoryName?: string): ProductFaqItem[] {
  ...
  faqs.push({
    ...
    answer:
      `Yes — the ${product.name} is ${ratingDesc} by Aussie Vape Hub customers, averaging ${product.rating.toFixed(
        1
      )}/5 across ${product.reviewCount.toLocaleString()} reviews. ${product.shortDescription} It's stocked authentic at Aussie Vape Hub and dispatched same-day from our Sydney warehouse.`,
  })
  ...
  if (isPack && packSize) {
    faqs.push({
      ...
      answer:
        `The ${product.name} includes ${packSize}× ${product.brand} devices in one bundle, dispatched together from the Aussie Vape Hub Sydney warehouse. Pack pricing saves you ${
  ...
  faqs.push({
    ...
    answer:
      `Aussie Vape Hub is the largest authorised AU stockist of ${product.brand} products. Every ${product.name} sold by Aussie Vape Hub is sourced through authorised distribution channels, carries the brand's authenticity verification code, and is dispatched same-day from our Sydney warehouse on weekday orders before 2pm AEST. Free shipping on orders over $300.`,
  })
  ...
  faqs.push({
    ...
    answer:
      `Yes — the ${product.name} sold by Aussie Vape Hub is TGA-compliant 20mg/mL (2%) salt nicotine and operates within Australia's prescription model for nicotine vaping products. You confirm prescription status at checkout.`,
  })
