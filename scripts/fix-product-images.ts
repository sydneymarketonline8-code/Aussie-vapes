...
async function main() {
  ...
  for (const row of imageRows) {
    ...
    if (onDiskName) {
      fixes.push({ id: row.id, from: row.url, to: `https://www.aussievapehub.com.au/products/${onDiskName}` })
      extensionFixed++
      continue
    }
    ...
    if (fuzzy) {
      fixes.push({ id: row.id, from: row.url, to: `https://www.aussievapehub.com.au/products/${fuzzy}` })
      extensionFixed++
      continue
    }
  ...
  for (const p of productsWithoutImage) {
    ...
    if (onDiskName) {
      inserts.push({ product_id: p.id, url: `https://www.aussievapehub.com.au/products/${onDiskName}`, position: 0 })
      backfilled++
      continue
    }
    const fuzzy = fuzzyMatch(p.slug, idx)
    if (fuzzy) {
      inserts.push({ product_id: p.id, url: `https://www.aussievapehub.com.au/products/${fuzzy}`, position: 0 })
      backfilled++
      continue
    }
