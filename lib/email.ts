 ...
 *
 * RESEND_FROM controls the From address; defaults to "Aussie Vape Hub <orders@vapehubvapesaustralia.com.au>".
...
export async function sendEmail(msg: EmailMessage): Promise<{ ok: boolean; error?: string }> {
  ...
  const from = process.env.RESEND_FROM ?? 'Aussie Vape Hub <orders@vapehubvapesaustralia.com.au>'
