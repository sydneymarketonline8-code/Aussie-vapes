...
export const metadata: Metadata = {
  title: 'VapeHub Vapes Australia Sitemap — All Pages, Brands & Categories',
  description: 'Complete sitemap of VapeHub Vapes Australia. Browse every category, brand, location and information page in one place.',
...
const RESOURCES = [
  { label: 'VapeHub Vapes Australia FAQ', href: '/faq' },
...
const ACCOUNT = [
  ...
  { label: 'Search VapeHub Vapes Australia', href: '/search' },
]
    ...
    <>
      <section className="bg-soft-100 border-b border-line">
        <div className="container-site py-10">
          <Breadcrumb crumbs={[{ label: 'VapeHub Vapes Australia', href: '/' }, { label: 'Sitemap' }]} />
          <h1 className="font-display text-4xl lg:text-5xl font-bold text-ink mt-4 mb-3 lowercase">
            vapehub vapes australia sitemap
          </h1>
          <p className="text-body max-w-2xl leading-relaxed">
            Every page on VapeHub Vapes Australia, organised. The full XML sitemap for search engines lives at{' '}
      ...
      <section className="container-site py-14 space-y-12">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-8">
          ...
          <Col title="My VapeHub Vapes Australia" items={ACCOUNT} />
          <Col title="Policies" items={POLICIES} />
        </div>

        <div>
          <h2 className="font-display text-2xl font-bold text-ink mb-4 lowercase">vapehub vapes australia by location</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            <Link href="/vapehub-vapes-australia" className="px-4 py-3 bg-white border border-line rounded-sm font-display text-sm font-bold uppercase tracking-wider text-ink hover:bg-ink hover:text-white transition-colors text-center">
              All Locations
            </Link>
            {CITIES.map((c) => (
              <Link
        ...
        <div>
          <h2 className="font-display text-2xl font-bold text-ink mb-4 lowercase">all {BRANDS.length} vapehub vapes australia brands</h2>
